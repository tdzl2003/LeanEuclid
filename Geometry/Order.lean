...
namespace Geometry.HilbertAxioms2D
  ...
  section
    ...
    theorem HilbertAxioms2D.between_exists(a c: Point): a ≠ c → ∃ b: Point, Between a b c := by
      sorry
    ...
    theorem HilbertAxioms2D.between_trichotomy(a b c: Point): Collinear a b c → a ≠ b → b ≠ c → a ≠ c →
